// Tailwind color palette mapped to hex values
export const colors = {
  // Grays
  gray: {
    50: '#f9fafb',
    100: '#f3f4f6',
    200: '#e5e7eb',
    300: '#d1d5db',
    400: '#9ca3af',
    500: '#6b7280',
    600: '#4b5563',
    700: '#374151',
    800: '#1f2937',
    900: '#111827',
  },
  // Blues
  blue: {
    50: '#eff6ff',
    100: '#dbeafe',
    200: '#bfdbfe',
    300: '#93c5fd',
    400: '#60a5fa',
    500: '#3b82f6',
    600: '#2563eb',
    700: '#1d4ed8',
    800: '#1e40af',
    900: '#1e3a8a',
  },
  // Greens
  green: {
    50: '#f0fdf4',
    100: '#dcfce7',
    200: '#bbf7d0',
    300: '#86efac',
    400: '#4ade80',
    500: '#22c55e',
    600: '#16a34a',
    700: '#15803d',
    800: '#166534',
    900: '#14532d',
  },
  // Emeralds
  emerald: {
    50: '#ecfdf5',
    100: '#d1fae5',
    200: '#a7f3d0',
    300: '#6ee7b7',
    400: '#34d399',
    500: '#10b981',
    600: '#059669',
    700: '#047857',
    800: '#065f46',
    900: '#064e3b',
  },
  // Yellows/Ambers
  amber: {
    50: '#fffbeb',
    100: '#fef3c7',
    200: '#fde68a',
    300: '#fcd34d',
    400: '#fbbf24',
    500: '#f59e0b',
    600: '#d97706',
    700: '#b45309',
    800: '#92400e',
    900: '#78350f',
  },
  // Reds
  red: {
    50: '#fef2f2',
    100: '#fee2e2',
    200: '#fecaca',
    300: '#fca5a5',
    400: '#f87171',
    500: '#ef4444',
    600: '#dc2626',
    700: '#b91c1c',
    800: '#991b1b',
    900: '#7f1d1d',
  },
  // Indigos
  indigo: {
    50: '#eef2ff',
    100: '#e0e7ff',
    200: '#c7d2fe',
    300: '#a5b4fc',
    400: '#818cf8',
    500: '#6366f1',
    600: '#4f46e5',
    700: '#4338ca',
    800: '#3730a3',
    900: '#312e81',
  },
} as const;

// Chart-specific color mappings
export const chartColors = {
  // Bar chart colors
  invoiced: colors.blue[500],      // #3b82f6
  paid: colors.emerald[500],        // #10b981
  
  // Text colors
  primary: colors.gray[700],        // #374151
  secondary: colors.gray[500],      // #6b7280
  
  // Status colors
  success: colors.emerald[500],     // #10b981
  warning: colors.amber[500],       // #f59e0b
  error: colors.red[500],           // #ef4444
  
  // Background colors
  grid: colors.gray[100],           // #f3f4f6
  tooltip: {
    background: 'rgba(0, 0, 0, 0.9)',
    border: colors.gray[700],       // #374151
  },
  
  // Shadow
  shadow: '0 4px 12px rgba(0, 0, 0, 0.15)',
} as const;

// Helper function to get color by Tailwind class name
export const getColor = (colorName: string): string => {
  const [color, shade] = colorName.split('-');
  if (shade && color in colors) {
    const colorObj = colors[color as keyof typeof colors];
    if (shade in colorObj) {
      return colorObj[shade as unknown as keyof typeof colorObj];
    }
  }
  return colorName; // Return original if not found
}; 